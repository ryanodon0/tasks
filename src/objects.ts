...
export function isValid(question: Question, answer: string): boolean {
    if (question.type == "short_answer_question") {
        ...
        return question.options.some(
            (answerOption: string): boolean => answerOption == answer,
        );
    }
}
...
export function toMarkdown(question: Question): string {
    let questionPrint: string = "# " + question.name + "\n" + question.body;
    if (question.type == "multiple_choice_question") {
        questionPrint += "\n- " + question.options.join("\n- ");
    }
    return questionPrint; // ADD AN IF STATEMENT FOR IF THE QUESTION TYPE IS MULTIPLE CHOICE
}
...
export function addOption(question: Question, newOption: string): Question {
    ...
    updatedOptions.options.push(newOption);
